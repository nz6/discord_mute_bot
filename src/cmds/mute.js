...
async function run(bot, message, args) {
  ...
  if (!memberRole || !toMute.roles.has(memberRole.id)) logger.log('This user does not have the Memeber role!');

  // Remove the mentioned users role "memberRole"
  await toMute.removeRole(memberRole);
}
